...
// which are part of the Xenios repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="xenios-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /xenios-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "xeniospay-xenios/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from xeniospay-xenios/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'xeniospay-xenios/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from xeniospay-xenios/develop"
            }
          }

          if (hasCache) {
            sh "cd /xenios-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /xenios-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /xenios-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /xenios-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /xenios-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /xenios-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /xenios-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
