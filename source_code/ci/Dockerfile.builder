...
# xenios_hash
RUN git clone https://github.com/xeniospay/xenios_hash
RUN cd xenios_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} xenios
RUN useradd -u ${USER_ID} -g xenios -s /bin/bash -m -d /xenios xenios
...
RUN mkdir /xenios-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /xenios-src && \
