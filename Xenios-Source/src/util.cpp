...
const char * const BITCOIN_CONF_FILENAME = "xenios.conf";
const char * const BITCOIN_PID_FILENAME = "xeniosd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL || *cacheCounter != logAcceptCategoryCacheCounter.load())
        {
            ...
            if (mapMultiArgs.count("-debug")) {
                ...
                // "xenios" is a composite category enabling all Xenios-related debug output
                if(ptrCategory->count(std::string("xenios"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Xenios
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Xenios
    // Mac: ~/Library/Application Support/Xenios
    // Unix: ~/.xenios
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Xenios";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Xenios";
#else
    // Unix
    return pathRet / ".xenios";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty xenios.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override xenios.conf
